...
if (strpos($referer, "checkout.php") !== false) {
    ...
    $prodId = 34732;
} elseif (strpos($referer, "stdProductAndHealthUpsell.php") !== false) {
    $prodId = $_POST['productId'];
}
...
if (isset($responseSession->Transaction->OrderInfo)) {
    $_SESSION['orderId'] = $responseSession->Transaction->OrderInfo->OrderID;
}

// logToFileMain(orderComplete());

echo $response;


// logToFileMain($postData);
// logToFileMain(json_encode($responseSession));

// function logToFileMain($var)
// {
//     ob_flush();
//     ob_start();
//     var_dump($var);
